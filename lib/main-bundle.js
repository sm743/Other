...
function checkMill(move, start, end, checkRow, gameProperties) {
    ...
    if (count === 3) {
        ...
        for (var _i = start; _i <= end; _i++) {
            ...
            if (_tileState.ISAVAILABLE === true) {
                _tileState.ISMILL++;
                for (var j = start; j <= end; j++) {
                    var otherState = checkRow ? move.BOARD[move.ROW][j] : move.BOARD[j][move.COL];
                    if (otherState.ISAVAILABLE === true) {
                        if (_i !== j) {
                            if (_tileState.OTHER_MILLS === undefined || _tileState.OTHER_MILLS === null) {
                                _tileState.OTHER_MILLS = [];
                            }

                            if (checkRow) {
                                _tileState.OTHER_MILLS.push({ ROW: move.ROW, COL: j });
                            } else {
                                _tileState.OTHER_MILLS.push({ ROW: j, COL: move.COL });
                            }
                        }
                    }
                }
...
function getQueryString() {
    var ampher = window.location.search.slice(1).split("&");
    var query_string = {};
    for (var i = 0; i < ampher.length; i++) {
        var kv = ampher[i].split("=");
...
function checkLose(gameProperties) {
    return gameProperties.PURPLE_PLAYER.AVAILABLE === 0 && gameProperties.YELLOW_PLAYER.AVAILABLE === 0 && (gameProperties.PURPLE_PLAYER.PLACED < 3 || gameProperties.YELLOW_PLAYER.PLACED < 3);
}
...
function scoreBoard(turn, maxPlayer, gameProperties, depth) {
    ...
    if (checkLose(gameProperties) && (turn === common.YELLOW_TURN && gameProperties.PURPLE_PLAYER.PLACED < 3 || turn === common.PURPLE_TURN && gameProperties.YELLOW_PLAYER.PLACED < 3)) {
        score = 100000 + depth;
    } else {
        // Random number from 0 to 1 to choose between any equal board score randomly
        // let r = Math.random();
        score = turn === common.YELLOW_TURN ? gameProperties.YELLOW_PLAYER.PLACED - gameProperties.PURPLE_PLAYER.PLACED : gameProperties.PURPLE_PLAYER.PLACED - gameProperties.YELLOW_PLAYER.PLACED;
    }
                        ...
                        for (var i = 0; i < MATRIX_SIZE; i++) {
                            for (var j = 0; j < MATRIX_SIZE; j++) {
                                if (board[i][j].ISAVAILABLE && board[i][j].TURN === null) {
                                    ...
                                    if (_copyBoard[row][col].ISMILL > 0) {
                                        _copyBoard[row][col].ISMILL--;
                                        ...
                                        if (_copyBoard[row][col].ISMILL === 0) {
                                            _copyBoard[row][col].OTHER_MILLS = [];
                                        }
                                        ...
                                        if (mills !== null && mills !== undefined) {
                                            ...
                                            for (var k = 0; k < mills.length; k++) {
                                                // invalidate mills
                                                if (_copyBoard[mills[k].ROW][mills[k].COL].ISMILL > 0) {
                                                    _copyBoard[mills[k].ROW][mills[k].COL].ISMILL--;
                                                    turn === common.YELLOW_TURN ? _copyGameProperties.YELLOW_PLAYER.MILLPIECES-- : _copyGameProperties.PURPLE_PLAYER.MILLPIECES--;
                                                }

                                                if (_copyBoard[mills[k].ROW][mills[k].COL].ISMILL === 0) {
                                                    _copyBoard[mills[k].ROW][mills[k].COL].OTHER_MILLS = [];
                                                }
                                            }
                                        }
                                    }
    ...
    while (numMills > 0) {
        ...
        for (var row = 0; row < MATRIX_SIZE; row++) {
            for (var col = 0; col < MATRIX_SIZE; col++) {
                ...
                move.COL = col;
                if (algorithm.isRemovable(move, otherTurn)) {
                    if (removeMillPiece && move.BOARD[move.ROW][move.COL].ISMILL > 0 || !removeMillPiece && move.BOARD[move.ROW][move.COL].ISMILL === 0) {
...
var depth = 3; // TODO seems like the max reasonable depth is 4, but 3 works pretty fast

if (window.location.search.includes("depth")) {
    depth = (0, _utils.getQueryString)()["depth"];
}
